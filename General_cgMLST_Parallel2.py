...
def PSL_Line_Lister(input_PSL):
    Out = []
    f = open(input_PSL, 'r')
    for line in f:
        List1 = line.split('\t')
        if int(List1[0]) > (int(List1[14]) / 2):
            Out.append(line)
    f.close()
    return Out

def PSL_Line_Allele_Matcher(input_list):
##    (PSL_line, Isolate_Genome, allele_folder)
    PSL_line = input_list[0]
    Isolate_Genome = input_list[1]
    allele_folder = input_list[2]
    List1 = PSL_line.split('\t')
    PA_Gene_Name = List1[13][0:-2]
    Isolate_Gene_Name = List1[9]
    if List1[8] == '+':
        Gene_Match = Isolate_Genome[Isolate_Gene_Name][int(List1[11]):int(List1[12])]
        Data = Single_Gene_Match_2(Gene_Match, allele_folder + PA_Gene_Name + '_alleles.fasta')
    else:
        Sequenced_Gene = Isolate_Genome[Isolate_Gene_Name][int(List1[11]):int(List1[12])]
        Rev_Gene = Sequenced_Gene.seq.reverse_complement()
        Gene_Match = SeqRecord(Seq(str(Rev_Gene)))
...
def PSL_Allele_Match_List(input_PSL, input_fasta, allele_folder):
    ...
    Lines = PSL_Line_Lister(input_PSL)
    Inputs = []
    for line in Lines:
        Inputs.append([line, Genome, allele_folder])
    with Pool() as pool:
        Matches = pool.map(PSL_Line_Allele_Matcher, Inputs)
    Matches.sort()
    return Matches
